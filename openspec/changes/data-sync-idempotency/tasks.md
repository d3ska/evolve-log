...
- [ ] **T1** Write `V21__data_sync_idempotency.sql`:
...
- [ ] **T2** Update `BloodTestService` upload logic:
...
- [ ] **T3** Update `FitatuImportService` to use native upsert:
  `ON CONFLICT (user_id, date, meal, food_name) DO UPDATE SET quantity_g=..., nutrients=..., imported_at=now()`

## Phase 4: Tests

- [ ] **T4** Integration test: upload blood test CSV twice → single report row, result rows updated
- [ ] **T5** Integration test: re-import Fitatu CSV → rows updated, no duplicates
- [ ] **T6** Integration test: upload blood test CSV with null lab_name twice → single row
