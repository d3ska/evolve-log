...
Defines the minimum automated test coverage standards, using the supplement domain as the
pilot pattern for service-layer unit tests and controller-layer MockMvc tests.
...
### Requirement: SupplementController has MockMvc coverage for all endpoints
The system SHALL have MockMvc tests for all `SupplementController` endpoints verifying
status codes, request parsing, and service delegation (using `standaloneSetup` — `@WebMvcTest`
was removed in Spring Boot 4.x).
...
The system SHALL include a `TEST_COVERAGE.md` listing all domains not yet covered by
automated tests, as a roadmap for subsequent test phases.
